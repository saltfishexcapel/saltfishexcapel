...
help:
	...
	{
		printf("C词斩主程序启动，版本:v1.0-fix211228\n\n");
		...
		for(i=0;i<=50;i++)
		{
			cDefaultName[i]=fgetc(cword_config);
			/*printf("%d%c\n",i,cDefaultName[i]);*/
		...
		//printf("debug: cDefaultName “%s”\n",cDefaultName);
		/*注意，下面这条语句同时以r+打开了cDefaultName描述的文件*/
		if((fDefaultFile=fopen(cDefaultName,"r+")) == NULL) /*配置文件中没有找到对应文件*/
		{
			//printf("%s\n",cDefaultName);
...
/*C词斩主程序部分*/
	static char cInput;
	int iCorrect=0;
	int iInputcharInstruction=0;
	printf("欢迎来到C词斩！\n输入w录入单词，输入s回忆单词，输入q退出:");
	while(iCorrect!=1)
	{
		//printf("debug: 01(int)cInput:%d\n",(int)cInput);
		if(iInputcharInstruction==0)
		{
			cInput=getchar(); /*输入*/
		}
		//printf("debug: 02(int)cInput:%d\n",(int)cInput);
		if(cInput == 'w')
		{
			if(argc == 1) /*把文件指针传入函数*/
			{
				Write(&fDefaultFile); /*调用Write函数执行相关功能*/
			}
			else
			{
				Write(&fFilename);
		...
		else if(cInput == 's')
		{
			if(argc == 1)
			{
				iCorrect=Show(&fDefaultFile); /*调用Show函数执行相关功能*/
			}
			else
			{
			...
			{
				//printf("debug: test\n");
				iInputcharInstruction=1;
				cInput = 'w';
			}
			//iCorrect=1;
			//printf("debug: 01 cInput:%c\n",cInput);
		}
		else if(cInput == 'q')
		{
			goto exit;
